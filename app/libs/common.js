...
(function(exports) {
    ...
    exports.readExtension = function(filename) {
        return /(?:\.([^.]+))?$/.exec(filename)[1];
    }
    
    exports.formatFile = function(file) {
        var msgStr = "";
        if(file) {
            ...
            msgStr += '<p><a id="c'+file.servername+'link" href="'+encodeURIComponent(file.url)+'" target="_blank">'
                    ...
                    +'</span></p>';
            msgStr += this.getViewer(file);
        }
        return msgStr;
    }

    exports.getViewer = function(file) {
        if((/\.(gif|jpg|jpeg|tiff|png)$/i).test(file.originalname)) {
            return '<img class="image-small" src="'+file.url+'" alt="'+file.originalname+'" onclick=$(this).toggleClass("image-small");></img>';
            ...
            return '<embed flashvars="file='+file.url+'" allowfullscreen="true" allowscriptaccess="always" '
                  +'id="'+file.servername+'_video" src="/static/lib/jwplayer/player.swf" width="640" height="363"/>';
        }
        return '';
    }
    ...
    exports.processMessage = function(message) {
        var items = [];
        message = this.htmlentities(message, 'ENT_NOQUOTES');
        ...
        var replacer = function(_, url, offset, string) {
            if(/^https?:\/\/(?:www\.)?youtube.com\/watch\?(?=.*v=\w+)(?:\S+)?$/.test(url)) { // youtube
                if(items.length <= 5) // limit to 5 objects per message
                    items.push('<p>'+exports.embedYoutube(url)+'</p>');
            }
    ...
    exports.embedYoutube = function(link, size) {
        size = size || { w: 640, h: 363 };
        var strId = link.match(/v=[\w-]{11}/)[0].replace(/v=/,'')
        var result = '<div class="embedded-video">\n';
        result += '<object width="'+size.w+'" height="'+size.h+'">\n';
        ...
        result += '<embed src="http://www.youtube.com/v/' + strId + '&hl=en&fs=1&" type="application/x-shockwave-flash" allowscriptaccess="always" allowfullscreen="true" width="'+size.w+'" height="'+size.h+'"></embed>\n';
        result += '</object>\n';
        result += '</div>\n';
        return result;
    }
    ...
    exports.dateStr = function(dateObj) {
        dateObj = dateObj || new Date();
        return '<span class="msgdate">'+this.date('j M Y, g:i:s a', (dateObj.getTime()/1000))+'</span> ';
    }
